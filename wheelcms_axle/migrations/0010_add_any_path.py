...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Provide a default 'any' path """
        for node in orm.Node.objects.all():
            any_path = node.paths.filter(language="any")
            if not any_path.exists():
                path = node.paths.all()[0].path
                orm.Paths(node=node, language="any", path=path).save()
    ...
    models = {
        ...
        'wheelcms_axle.content': {
            ...
            'expire': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2033, 8, 30, 0, 0)', 'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'wheelcms_axle.node': {
            ...
            'tree_path': ('django.db.models.fields.CharField', [], {'default': "'0x2a7db437a7e1eb732L'", 'unique': 'True', 'max_length': '255'})
        },
        'wheelcms_axle.paths': {
